...
# --- Logging Setup ---
LOG_FILE = BASE_PREPROCESSED_DIR / f"data_quality_{COUNTRY}_all.log" # Log file at BASE level
...
def load_json_files(directory: Path, zip_code: str) -> list: # Added zip_code parameter
    ...
    for file_path in directory.rglob("*.json"):
        ...
        try:
            ...
            json_objects.append({"filename": str(file_path.relative_to(BASE_RAW_DIR)), "data": data, "zip_code": zip_code}) # Add zip code here
...
def flatten_main_profile(profile_item: dict) -> dict: # Takes the item dict now
    ...
    flat_profile = {"source_filename": filename, "zip_code": zip_code, "_flattening_error": False} # Add zip_code
    try:
        ...
        flat_profile.update({
            ...
            "member_address_zip": address.get("zip"), # Note: zip from address vs zip_code from folder might differ
...
def extract_and_flatten_nested_profiles(main_profile_row: pd.Series) -> dict[str, list[dict]]:
    """
                ...
                for field, value in sub_profile.items():
                    ...
                    if field in dynamic_inner_dicts and isinstance(value, dict):
                        ...
                        for inner_key, inner_val in value.items():
                            ...
                            if isinstance(inner_val, bool) and inner_val: # Only store True flags? Or store key names? Let's store key names if True.
                        ...
                        flat_sub[f"{col_name_base}_flags_json"] = safe_json_dump(inner_vals_list) # Changed column name for clarity
                    ...
                    # 4. Handle simple, explicitly known fields (including booleans directly)
                         ...
                         flat_sub[col_name_base] = value # Booleans from JSON are already Python bools
...
def flatten_review(review_item: dict) -> dict: # Takes the item dict now
    ...
    flat_review = {"source_filename": filename, "zip_code": zip_code, "_flattening_error": False} # Add zip_code
...
def create_dataframe(item_objs: list, flatten_func: callable, desc: str) -> pd.DataFrame: # Takes list of items now
    ...
    if "_flattening_error" in df.columns: df = df.drop(columns=["_flattening_error"])
...
def create_reviews_dataframe(item_objs: list) -> pd.DataFrame: # Takes list of items
    ...
    for item in item_objs: # item is {"filename":..., "data":..., "zip_code":...}
        ...
        try:
            data_level=item.get('data',{}); payload_level=data_level.get('data',{}); review_payload=payload_level.get('reviewsByReviewee',{}); reviews_list=review_payload.get('reviews',[])
            ...
            for review_obj in reviews_list:
                if isinstance(review_obj, dict):
                    ...
                    review_item_data = {"filename": filename, "zip_code": zip_code, "data": review_obj}
                    flat_review = flatten_review(review_item_data) # Pass the constructed item
                    if isinstance(flat_review, dict) and not flat_review.get("_flattening_error"): 
...
if __name__ == "__main__":
        ...
        if zip_dir.is_dir() and zip_dir.name.isdigit():
            ...
            if current_reviews_input_dir.is_dir():
                for type_dir in current_reviews_input_dir.iterdir():
                 ...
                 logger.warning(f"Reviews directory not found for zip {zip_code}: {current_reviews_input_dir}")
    ...
    if all_profile_json_items:
        main_profiles_df = create_dataframe(all_profile_json_items, flatten_main_profile, "all_main_profiles")
        if not main_profiles_df.empty:
            ...
            if "profile_id" in main_profiles_df.columns:
                ...
                if dropped_null > 0: logger.info(f"Dropped {dropped_null} profiles missing profile_id before deduplication.")

                # Now drop duplicates based on profile_id, keeping the first
                initial_count_after_null = len(main_profiles_df)
                main_profiles_df.drop_duplicates(subset=["profile_id"], keep='first', inplace=True)
                dropped_duplicates = initial_count_after_null - len(main_profiles_df)
                if dropped_duplicates > 0:
                    logger.warning(f"Removed {dropped_duplicates} duplicate profiles based on 'profile_id'. Kept first occurrence.")
            else:
                logger.error("Column 'profile_id' not found! Cannot deduplicate profiles.")
            # --- END DEDUPLICATION STEP ---
            # Run checks and save at COUNTRY level
            logger.info("333 out")
            main_profiles_df = run_quality_checks_and_report(main_profiles_df, main_profile_schema, "main_profiles", COUNTRY_PREPROCESSED_DIR)

            # Extract nested profiles from the AGGREGATED main dataframe
            logger.info("Extracting and flattening nested profiles from aggregated main profiles...")
                ...
                for idx, row in main_profiles_df.iterrows():
                     try:
                        ...
                        for sub_type, flat_dict_list in nested_data_for_row.items():
                            if sub_type not in nested_profiles_data: nested_profiles_data[sub_type] = []
                            nested_profiles_data[sub_type].extend(flat_dict_list)
                     except Exception as nested_err: logger.error(f"Err extract nested agg {row.get('profile_id','N/A')}: {nested_err}\n{traceback.format_exc()}")
            ...
            if nested_profiles_data:
                ...
                for sub_type, list_of_dicts in nested_profiles_data.items():
                    if list_of_dicts:
                        logger.info(f"--- Creating DataFrame and running checks for aggregated: nested_{sub_type} ---")
                        try:
                            df_for_type = pd.DataFrame(list_of_dicts); df_for_type = df_for_type.convert_dtypes()
                            # --- DEDUPLICATION STEP FOR NESTED PROFILES ---
                            # Check duplicates based on profile_id and sub_profile_type
                            if "profile_id" in df_for_type.columns and "sub_profile_type" in df_for_type.columns:
                                 initial_nested_count = len(df_for_type)
                                 df_for_type.drop_duplicates(subset=["profile_id", "sub_profile_type"], keep='first', inplace=True)
                                 dropped_nested_dupes = initial_nested_count - len(df_for_type)
                                 if dropped_nested_dupes > 0:
                                     logger.warning(f"Removed {dropped_nested_dupes} duplicate nested profiles (profile_id, sub_profile_type) for type '{sub_type}'.")
                            # --- END DEDUPLICATION STEP ---
                            df_name = f"nested_{sub_type}"
                            # Save at COUNTRY level
                            run_quality_checks_and_report(df_for_type, nested_profile_schema, df_name, COUNTRY_PREPROCESSED_DIR)
                            logger.info(f"--- Finished checks for aggregated: {df_name} ---")
                        except Exception as type_proc_err: logger.error(f"Error processing aggregated sub_profile_type '{sub_type}': {type_proc_err}\n{traceback.format_exc()}")
                    else: logger.warning(f"No aggregated data found for sub_profile_type '{sub_type}'.")
            else: logger.warning("No nested profile data was extracted or aggregated.")
        else: logger.warning("Aggregated Main profiles DataFrame empty.")
    else: logger.warning(f"No profile JSON files found across all zip codes.")
    ...
    if all_review_json_items:
        ...
        if not reviews_df.empty:
            ...
            if "review_id" in reviews_df.columns:
                ...
                if dropped_r_null > 0: logger.info(f"Dropped {dropped_r_null} total reviews missing review_id before deduplication.")
                ...
                if dropped_r_duplicates > 0:
                    logger.warning(f"Removed {dropped_r_duplicates} duplicate reviews based on 'review_id'. Kept first occurrence.")
            else:
                logger.error("Column 'review_id' not found! Cannot deduplicate reviews.")
            # --- END DEDUPLICATION STEP ---
            # Run checks and save at COUNTRY level
            reviews_df = run_quality_checks_and_report(reviews_df, review_schema, "reviews", COUNTRY_PREPROCESSED_DIR)
        else: logger.warning("Aggregated Reviews DataFrame empty.")
    else: logger.warning(f"No review JSON files found across all zip codes.")

    logger.info("--- Proposed Data Model ---"); print_proposed_data_model()
