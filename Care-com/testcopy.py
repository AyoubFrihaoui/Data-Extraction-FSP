...
if __name__ == "__main__":
    ...
    # REMOVED: nested_profiles_df = pd.DataFrame() # We'll create separate DFs
    ...
    if profile_json_files:
        ...
        if not main_profiles_df.empty:
            ...
            # Initialize dict to aggregate results by sub_profile_type
            aggregated_nested_profiles: dict[str, list[dict]] = {}

            logger.info("Extracting and flattening nested profiles...")
            for idx, row in main_profiles_df.iterrows():
                 try:
                    # Function returns a dict: {sub_type: [flat_dict], ...}
                    nested_data_for_row = extract_and_flatten_nested_profiles(row) # Use the previously modified function
                    # Merge results into the aggregated dictionary
                    for sub_type, flat_dict_list in nested_data_for_row.items():
                        if sub_type not in aggregated_nested_profiles:
                            aggregated_nested_profiles[sub_type] = []
                        aggregated_nested_profiles[sub_type].extend(flat_dict_list)

                 except Exception as nested_err:
                    profile_id_err = row.get('profile_id', 'N/A')
                    filename_err = row.get('source_filename', 'N/A')
                    logger.error(f"Error during nested profile extraction aggregation for profile_id {profile_id_err} from {filename_err}: {nested_err}\n{traceback.format_exc()}")

            # --- CHANGED: Process and run quality checks for each sub_profile_type separately ---
            if aggregated_nested_profiles:
                logger.info("Processing aggregated nested profiles by type...")
                validated_nested_dfs = {} # Optional: Store validated DFs if needed later

                for sub_type, list_of_dicts in aggregated_nested_profiles.items():
                    if list_of_dicts:
                        logger.info(f"--- Creating DataFrame and running checks for: nested_{sub_type} ---")
                        try:
                            # Create DataFrame for this specific sub_profile_type
                            df_for_type = pd.DataFrame(list_of_dicts)
                            logger.info(f"Created df_for_type '{sub_type}' with {len(df_for_type)} rows and {len(df_for_type.columns)} columns.")

                            # Apply intelligent type conversion
                            logger.info(f"Applying convert_dtypes() to df_for_type '{sub_type}'...")
                            df_for_type = df_for_type.convert_dtypes()

                            # Define the name for reports and files for this specific type
                            df_name = f"nested_{sub_type}"

                            # Run quality checks using the specific name
                            # NOTE: Ensure 'nested_profile_schema' is appropriate for all types
                            # or consider using type-specific schemas if validation needs differ significantly.
                            validated_df = run_quality_checks_and_report(
                                df_for_type,
                                nested_profile_schema, # Pass the potentially generic or adaptable schema
                                df_name,               # Pass the specific name (e.g., "nested_childCareCaregiverProfile")
                                PROFILES_OUTPUT_DIR
                            )
                            validated_nested_dfs[sub_type] = validated_df # Store the result
                            logger.info(f"--- Finished checks for: {df_name} ---")

                        except Exception as type_proc_err:
                             logger.error(f"Error processing or running checks for sub_profile_type '{sub_type}': {type_proc_err}\n{traceback.format_exc()}")
                    else:
                        logger.warning(f"No data found for sub_profile_type '{sub_type}'. Skipping DataFrame creation and checks.")
            else:
    ...
    # --- Process Reviews (Remains the same) ---
    ...
    print_proposed_data_model() # Assuming this exists
...
def run_quality():
    ...
    # --- Process Reviews (Remains the same) ---
    logger.info(f"--- Processing Reviews ---")
    # ... (rest of your review processing code) ...
